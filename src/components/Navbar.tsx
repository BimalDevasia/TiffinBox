...
function navbar() {
  const data=[{id:"Home",link:"/home"},{id:"Menu",link:"menu"},{id:"Transactions",link:"transaction"},{id:"Profile",link:"profile"},{id:"Login",link:"login"},{id:"About",link:"about"}]
    ...
    <div className='fixed flex gap-16 mt-5 z-50 text-[18px] text-white'>
       {data.map((item,index)=>(
        ...
        <Link href={item.link} onClick={()=>handleClick(item.id)} >
         <div key={index}  className={`${activeItem===item.id ? "text-inyellow border-b-inyellow border-b-[3px] ":""}group z-50 relative hover:text-inyellow`}>{item.id}
